...
#define MAX_AMMO_VAR_SIZE 64
#define MAX_AMMO_PV_SIZE 32
	char ammo_var[MAX_AMMO_VAR_SIZE+1] = {0};
	char ammo_pv[MAX_AMMO_PV_SIZE+1] = {0};

	idme_get_oem_data_field("ammo_var=", ammo_pv, MAX_AMMO_PV_SIZE);
	if (!strlen(ammo_pv)) {
		printf("please check oem_data!\n");
		err = snprintf(ammo_var, MAX_AMMO_VAR_SIZE," androidboot.ammo.prod.var= ");
	} else {
		err = snprintf(ammo_var, MAX_AMMO_VAR_SIZE," androidboot.ammo.prod.var=%s ",ammo_pv);
	}
	if (err < 0) {
		printf("%s: snprintf error\n", __FUNCTION__);
	} else if (err >= MAX_AMMO_VAR_SIZE) {
		printf("%s: Truncated bootargs string\n", __FUNCTION__);
		free(new_str);
		return -FDT_ERR_TRUNCATED;
	}
	new_str_size += strlen(ammo_var) +1;
	new_str = realloc(new_str, new_str_size);
	if (!new_str) {
		printf("%s: Unable to allocate memory\n",__FUNCTION__);
		return -FDT_ERR_NOSPACE;
	}
	strncat(new_str, ammo_var, strlen(ammo_var) + 1);
	printf("set ammo var %s",ammo_pv);
#endif
