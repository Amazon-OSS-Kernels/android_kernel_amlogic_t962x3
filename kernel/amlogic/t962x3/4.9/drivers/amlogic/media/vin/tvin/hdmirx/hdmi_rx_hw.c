...
int dfe_test_en = 1;
u8 eq_fix_val = 16;
/*Enable CDR frequency tracking path. Active high.*/
int cdr_fr_en;
...
void aml_pll_bw_cfg_v2(void)
{
	...
	do {
		...
		udelay(5);
		if (idx <= PHY_BW_2)
			/* 1.sz sast dvd flashing screen issue,*/
			/* There is a large jitter between 1MHz*/
			/* and 2MHz of this DVD source.*/
			/* 2.philips dvd flahing when switch resolution */
			/* there will be a jitter when resolution changed*/
			/* it may cause phy unlock */
