...
bool is_unnormal_format(uint8_t wait_cnt)
{
	...
	if (rx.hdcp.hdcp_version == HDCP_VER_NONE) {
		...
		if (dev_is_appletv_v2) {
			if (wait_cnt == hdcp_none_wait_max * 20)
				ret = false;
...
int rx_set_global_variable(const char *buf, int size)
{
	...
	if (set_pr_var(tmpbuf, dfe_test_en, value, &index, ret))
		return pr_var(dfe_test_en, index);
	if (set_pr_var(tmpbuf, eq_fix_val, value, &index, ret))
		return pr_var(eq_fix_val, index);
	if (set_pr_var(tmpbuf, cdr_fr_en, value, &index, ret))
		return pr_var(cdr_fr_en, index);
	...
	case FSM_SIG_STABLE:
		...
		if (rx_is_timing_stable()) {
			if (++sig_stable_cnt >= sig_stable_max) {
				get_timing_fmt();
				/* stable, start count vsync and avi pkt */
...
void dump_aml_phy_sts(void)
{
	...
	//comb_val("    cdr0_int", cdr0_init, cdr1_init, cdr2_init, 7);
	/* status-6: pll lock */
