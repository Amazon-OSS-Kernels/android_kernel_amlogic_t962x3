...
static void amvecm_vadj_switch(struct am_vdj_mode_s *vdj_mode_s)
{
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_G12A) {
		...
		if (cur_vadj1_en != vadj1_en) {
			...
			pr_info("[amvecm.] vadj1 switch status unchanged.\n");
		}
		...
		if (cur_vadj2_en != vadj2_en) {
			...
			pr_info("[amvecm.] vadj2 switch status unchanged.\n");
		}
	} else {
		...
		unsigned int cmd, unsigned long arg)
{
	...
	struct pq_ctrl_s *pq_ctrl_val = NULL;
	...
	case AMVECM_IOC_S_PIC_MODE:
		...
		if (vdj_mode_flg & VDJ_FLAG_VADJ_EN) {
			...
			amvecm_vadj_switch(&vdj_mode_s);
			pr_info("[amvecm..] VADJ switch success.!!\n");
		}
	...
	case AMVECM_IOC_S_CMS_LUMA:
		if (copy_from_user(&data,
			...
			pr_amvecm_dbg("CMS luma parm read fail\n");
			ret = -EFAULT;
		} else {
			...
			pr_amvecm_dbg("CMS luma parm success\n");
		}
		break;
	case AMVECM_IOC_S_CMS_SAT:
		if (copy_from_user(&data,
			(void __user *)arg,
			sizeof(struct cms_data_s))) {
			pr_amvecm_dbg("CMS sat parm read fail\n");
			ret = -EFAULT;
		} else {
			...
			pr_amvecm_dbg("CMS sat parm success\n");
		}
		break;
	case AMVECM_IOC_S_CMS_HUE:
		if (copy_from_user(&data,
			(void __user *)arg,
			sizeof(struct cms_data_s))) {
			pr_amvecm_dbg("CMS hue parm read fail\n");
			ret = -EFAULT;
		} else {
			...
			pr_amvecm_dbg("CMS hue parm success\n");
		}
		break;
	case AMVECM_IOC_S_CMS_HUE_HS:
		if (copy_from_user(&data,
			(void __user *)arg,
			sizeof(struct cms_data_s))) {
			pr_amvecm_dbg("CMS hue by hs parm read fail\n");
			ret = -EFAULT;
		} else {
			...
			pr_amvecm_dbg("CMS hue by hs parm success\n");
		}
		break;
...
#define GAMMA_SETTING_CMDLINE "gamma_setting="

static int  gamma_read_cmdline(void)
{
	int ret = 0;
	char val_buf[64];
	char *gamma_setting = strstr(saved_command_line, GAMMA_SETTING_CMDLINE);

	if (gamma_setting) {
		ret = sscanf(gamma_setting,
		GAMMA_SETTING_CMDLINE"%s", val_buf);
		if (ret)
			pr_info("gamma_setting=%s\n", val_buf);
	}
